...
sudo apt-get -y install kitty seclists putty-tools powercat lolcat neofetch build-essential grc gcc cryptsetup steghide feroxbuster hping3 pipx zsh remmina keepass2 moreutils xclip ftp exploitdb locate netdiscover feh rdesktop snmp dirsearch docker-compose docker.io scrub jq apache2 ncat ntpdate rlwrap metasploit-framework ipcalc xsltproc swaks flameshot ghex hexedit
sudo apt-get update && sudo apt-get -y install golang-go 
sudo apt install libcriterion-dev clang
